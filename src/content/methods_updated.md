...
Running parallel to (and partly building on) the Swiss Cheese pipeline is a binary comparison that produces three flat lists of substances: approved in both countries, US only, or UK only. This was run twice with slightly different parameters, producing:
...
2. **Canonicalise units** — map variants to a standard form: `mg`, `g`, `mcg`, `%`, `mg/ml`, `mg/inh`, `mcg/inh`, `iu`, etc.
...
| Tier | Criteria |
|------|----------|
| **HIGH** | `inn_score == 1.0` AND `strength_score == 1.0` — exact same ingredients, numerically identical dose |
...
With the substance-level match table from Stage 4, the pipeline can now ask: for drugs that exist in both countries, is the legal access status the same? Or is something prescription-only in one country but available over the counter in the other?
...
**The three groups:**
- **Group A (Both countries):** 1,323 matched substance pairs — full regulatory discrepancy classification applied
...
- **Neutral fallback for unparseable strengths** — products with strength strings that couldn't be parsed receive a neutral score of 0.5, which quietly biases them to score slightly higher than products with parseable but mismatched strengths.
