...
This document traces every transformation the data goes through, from the original source spreadsheet to the files that power the website. It covers source acquisition, PDF extraction, normalisation, two distinct matching systems, and the legal discrepancy analysis.
...
```
Pillgram.xlsx  (master source workbook, 3 datasets)
      │
      ├─ FDA Orange Book sheet  ──────────────────────────┐
      └─ FOI2025_00058 All MAs sheet  ────────────────────┤
            (converted from a 680-page MHRA FOI PDF)      │
                                                           ▼
                                          v1 CSVs  (normalised_fda_orangebook.csv
                                                    normalised_uk_mhra.csv)
                                                           │
                                          normalise_pharma.py
                                                           │
                                                           ▼
                                          v2 CSVs  (normalised_fda_orangebook_v2.csv
                                                    normalised_uk_mhra_v2.csv)
                                                           │
                              ┌────────────────────────────┼───────────────────────┐
                              ▼                            ▼                       ▼
                    Swiss Cheese Pipeline          Binary Comparison        Similarity Scoring
                    (match_pipeline.py)            (compare_pipeline.py     (match_drugs.py)
                           │                        + match_drugs.py v2)          │
                           ▼                               │                      ▼
                  drug_match_results.csv          drugs_in_both.csv       fda_to_mhra_matches.csv
                    (1,323 matches)               drugs_fda_only.csv      mhra_to_fda_matches.csv
                           │                      drugs_mhra_only.csv
                           ▼
...
`Pillgram.xlsx` is the master source workbook for the entire project. It bundles three regulatory datasets — all acquired on 21 January 2026 — along with a `META` sheet recording provenance for each.
...
The central analysis is UK vs US: **MHRA vs FDA**. The Art57/EMA data is present as a reference but is not used in the matching pipeline.
...
The `Type` column contains `RX`, `OTC`, or `DISCN` (discontinued). Discontinued entries are a significant proportion of the dataset — approximately 45% of rows — and are handled carefully throughout the pipeline.

### The MHRA Dataset — Extracting Data from a PDF

The MHRA data was not available as a download. It came via a Freedom of Information request (FOI2025_00058), and MHRA supplied it as a 680-page landscape PDF — itself generated by "Print to PDF" from an original Excel spreadsheet. This made it an unusually difficult extraction target.

**The problem:** Because the PDF was generated from a printed page, the rendered x-coordinates of characters from adjacent columns physically overlap. Standard PDF table extraction tools (`pdfplumber`, `tabula-py`) produce garbled output because they assign characters to columns by x-position, causing characters from different columns to interleave.

**The key insight:** In the PDF's internal byte stream, each cell's characters are written as a contiguous block — even if their rendered positions overlap neighbours on screen. Two signals reliably detect column boundaries without relying on x-position:
...
**Final result: 29,226 rows, 8 columns, zero empty fields, zero bad authorisation numbers, zero bad dates.**

The extraction was independently replicated by a second Claude instance to verify reproducibility — it was confirmed replicable. The output CSV was then loaded into Pillgram.xlsx as the `FOI2025_00058 All MAs` sheet.

**MHRA data structure — multi-row products:** The MHRA format stores one row per active substance per product. A combination product therefore produces multiple rows sharing the same `Authorisation Number`. This structural difference from the FDA format (one row per product) is a recurring challenge throughout the pipeline.
...
RxNorm (NIH/NLM drug nomenclature) was used to cross-check substance names during this phase: 96.1% of MHRA substances and 95.4% of FDA substances resolved to an RxCUI, confirming that the core ingredient names were already widely recognised.
...
Run timestamp: 2026-02-25 13:36:30 (offline mode).
...
For names that didn't change in tiers 1 or 2, queries the ChEMBL REST API for a canonical preferred name. Slow (~30 minutes), caches results in `.chembl_cache.json`. **Was not used in the captured run** — the production v2 files were generated in offline mode only.
